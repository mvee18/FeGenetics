...
pub fn run_spectro(organism_id: String) -> String {
        ...
        let organism_path = PathBuf::from(format!("organisms/{}", organism_id)).canonicalize().unwrap();
        ...
        println!("The file paths are as follows: {} {}", spectro_path.display(), organism_path.display());

        // Run spectro from spectro path and pipe in the input file.
        let command = Command::new(&spectro_path).stdin(input_file)
